...
#        saliency_labels = np.concatenate((1 - self.labels, self.labels), axis = 1)
        saliency_map, self.sal_loss, self.sal_diff = self.sigmoid_crossentropyx_loss(saliency_map, saliency_labels)
...
#        self.loss = np.sum((tmp_foreground_example - tmp_background_example) ** 2) / self.num_cases
        ...
        if GV.phase == 'test':
            EPSILON = 1e-8
            tmp = np.array(saliency_map[:, 0, np.newaxis])
            bounduary = 20
...
#                dsadaf
            ...
            img.save(GV.target_dir + '/' + GV.data_name + '_images.jpg')
#            print GV.target_dir + '/' + GV.data_name + '_images.jpg'
#            print GV.data_name
            img = Image.fromarray(np.array(scale0_labels[0], dtype = np.uint8))
            img.save(GV.target_dir + '/' + GV.data_name + '_labels.jpg')
            img = Image.fromarray(np.array(result, dtype = np.uint8))
            img.save(GV.target_dir + '/' + GV.data_name + '_metric_result.jpg')
            img = Image.fromarray(np.array(sal, dtype = np.uint8))
            img.save(GV.target_dir + '/' + GV.data_name + '_sal_result.jpg')
            GV.test_nums += 1 
