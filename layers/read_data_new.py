...
#                asd         
                orignial_image_labels[np.where(orignial_image_labels>0)] = 1
...
#                image_data = np.concatenate((image_data, image_sobel[:,:,np.newaxis]), axis = 2)
                ...
                M=cv2.getRotationMatrix2D((self.resize_size[0]/2, self.resize_size[0]/2), degree,1)
                image_data=cv2.warpAffine(image_data,M,(self.resize_size[0],self.resize_size[1]))
                x = self.resize_size[0] * 1. / (2 + np.tan(degree/360*2*np.pi) + 1 / np.tan(degree/360*2*np.pi) + 1 / np.sin(degree/360*2*np.pi) + 1 / np.cos(degree/360*2*np.pi))
#                print x, (2 + np.tan(degree/360*2*np.pi) + 1 / np.tan(degree/360*2*np.pi) + 1 / np.sin(degree/360*2*np.pi) + 1 / np.cos(degree/360*2*np.pi))
#                print np.tan(degree/360*2*np.pi), np.sin(degree/360*2*np.pi), np.cos(degree/360*2*np.pi)
                x = int(x) + 1
                image_data = image_data[x:-x, x:-x]
...
#                image_labels[np.where(image_labels>0)] = 1
                image_labels=cv2.warpAffine(image_labels,M,(self.resize_size[0],self.resize_size[0]))
                image_labels = image_labels[x:-x, x:-x]
#                print x, degree
                image_data = imresize(np.array(image_data, dtype = np.uint8), [self.resize_size[0], self.resize_size[1], 3])
                image_labels = imresize(np.array(image_labels, dtype = np.uint8), [self.resize_size[0], self.resize_size[1]])
#                print self.resize_size[0], self.resize_size[1], height_widht_ratio
#                print [int(self.resize_size[0] *  1. * height_widht_ratio) , int(self.resize_size[1] * 1. / height_widht_ratio), 3], image_data.shape
    ...
    #            print 'hello', GV.data_name
                if len(image_data.shape) == 3:
                    if image_data.shape[2] != 3:
                        GV.image = image_data
                        image_data = imresize(np.array(image_data[:,:,0], dtype = np.uint8), [self.resize_size[0], self.resize_size[1]])
                        image_data = np.tile(image_data[:,:,np.newaxis], [1,1,3])
#                        image_sobel_0 = imresize(np.array(image_sobel, dtype = np.uint8), [self.resize_size[0], self.resize_size[1]])
#                        image_sobel_1 = imresize(np.array(image_sobel, dtype = np.uint8), [self.resize_size[0]/2, self.resize_size[1]/2])
#                        image_sobel_2 = imresize(np.array(image_sobel, dtype = np.uint8), [self.resize_size[0]/4, self.resize_size[1]/4])
