...
#                print osp.join(self.data_dir, self.train_labels_dir, self.train_images[self.train_timing].split('.jpg')[0] + '.png')                
                GV.data_name = self.train_images[self.train_timing].split('.jpg')[0] + '.png'
                ...
                if height_add_flag or width_add_flag:
                    ...
                    add_label = np.zeros([height * (1 + height_add_flag), width * (1 + width_add_flag), 3])
                    if height_add_flag and not width_add_flag:
                        add_image[:height,:width] = np.flipup(orignial_image_data)
                        add_image[height:2*height,:width] = orignial_image_data
                        add_label[:height,:width] = np.flipup(orignial_image_labels)
                ...
                image_data = orignial_image_data[start_x : start_x + self.resize_size[0], start_y : start_y + self.resize_size[1]]
#                image_sobel = orignial_image_sobel[end_x - int(height * tmp_crop_ratio) : end_x, end_y - int(width * tmp_crop_ratio) : end_y]
                image_labels = orignial_image_labels[start_x : start_x + self.resize_size[0], start_y : start_y + self.resize_size[1]]
